...
if (isset($_GET['q'])) {
    ...
    die();
}else if (isset($_GET['descuento'])) { $id = $_GET['id']; $desc_porcentaje = $_GET['desc']; // Este es el porcentaje que se va a descontar 
    $consulta = mysqli_query($conexion, "SELECT * FROM detalle_temp WHERE id = $id"); 
    $result = mysqli_fetch_assoc($consulta); 
    $total_desc = ($result['precio_venta'] * ($desc_porcentaje / 100)) * $result['cantidad']; // Calcular el descuento en base al porcentaje 
    $total = $result['total'] - $total_desc; // Restar el descuento del total 
    $insertar = mysqli_query($conexion, "UPDATE detalle_temp SET descuento = $total_desc, total = '$total' WHERE id = $id");
     if ($insertar) { $msg = array('mensaje' => 'descontado'); 
    }else{ $msg = array('mensaje' => 'error'); } echo json_encode($msg); 
    die();
...
if (isset($_POST['regDetalle'])) {
    ...
    $verificarExistencia = mysqli_query($conexion, "SELECT cantidad FROM producto WHERE codproducto = $id");
    $existencia = mysqli_fetch_assoc($verificarExistencia);
    ...
    if (!$existencia || $existencia['cantidad'] <= 0) {
        ...
        if ($cantidadSolicitada > $existencia['cantidad']) {
            ...
            if ($result > 0) {
                ...
                $query = mysqli_query($conexion, "UPDATE detalle_temp SET cantidad = $cantidadSolicitada, total = '$total_precio' WHERE id_producto = $id AND id_usuario = $id_user");
